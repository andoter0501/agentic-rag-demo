...
class MarkdownParser(DocumentParser):
    ...
    def extract_chunks(
        ...
        from .loader import _split_sections
...
class HTMLParser(DocumentParser):
    ...
    def parse(self, path: str | Path) -> list[Document]:
        ...
        headers = [
            h.get_text(strip=True) for h in soup.find_all(["h1", "h2", "h3", "h4", "h5", "h6"])
        ]
    ...
    def extract_chunks(
        ...
        for idx, para in enumerate(paragraphs, start=1):
            if len(bucket) + len(para) > max_chars and bucket:
                chunks.append(
                    ParsedChunk(
                        content=bucket,
                        metadata={"source": document.source, "chunk_index": idx},
        ...
        if bucket:
            chunks.append(
                ParsedChunk(
                    content=bucket,
                    metadata={"source": document.source, "chunk_index": len(chunks) + 1},
        ...
        if isinstance(data, dict):
            ...
            for idx, item in enumerate(data):
                ...
                chunks.append(
                    ParsedChunk(
                        content=chunk_content,
                        metadata={"source": document.source, "index": idx},
    ...
    def extract_chunks(
        ...
        for idx, line in enumerate(lines, start=1):
            if len(bucket) + len(line) > max_chars and bucket:
                ...
                bucket = line[-overlap:] if overlap > 0 and len(line) > overlap else line
            else:
...
class DocxParser(DocumentParser):
    def parse(self, path: str | Path) -> list[Document]:
        ...
        return [
            Document(
                content="\n".join(paragraphs),
                metadata={"source": str(path), "paragraphs": len(paragraphs)},
                source=str(path),
            )
        ]
